...
def filter_collection_s2(collection,region,cloud_percent):

    # Filter by location
    #region = ee.Geometry.Point(center_coord).buffer(14000).bounds()
    collection = collection.filter(
        ee.Filter.And(
            ee.Filter.gte('CLOUDY_PIXEL_PERCENTAGE', 0), ee.Filter.lte('CLOUDY_PIXEL_PERCENTAGE', cloud_percent)
        )
    )
...
def download_image_s2(collection,region,periods,save_root,point_id,center_coord,grid_coord):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--save-dir", type=str, default="./data/s2", help="dir to save data"
    )
    # collection properties
    parser.add_argument(
        "--collection", type=str, default="COPERNICUS/S2", help="GEE collection names"
    )
    parser.add_argument(
        "--cloud-pct", type=int, default=20, help="cloud percentage"
    )
    # patch properties
    parser.add_argument(
        "--dates",
    ...
    # get collection
    collection_s2 = ee.ImageCollection(args.collection)
    collection_s2 = collection_s2.filter(
    ee.Filter.And(
        ee.Filter.gte('CLOUDY_PIXEL_PERCENTAGE', 0), ee.Filter.lte('CLOUDY_PIXEL_PERCENTAGE', args.cloud_pct)
        )
    )
    ...
    def worker(idx):
        ...
        collection = collection_s2.filterBounds(region)
        collection = collection.filter(ee.Filter.contains('.geo', region))
        success_count = download_image_s2(collection,region,periods,args.save_dir,idx,center_coord,grid_coord)
